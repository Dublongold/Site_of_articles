"use strict";
async function check_search_options(event, checkbox_param) {
    var checkbox_elem = document.getElementById(checkbox_param[0]);
    if (checkbox_elem && checkbox_elem.checked !== null && checkbox_elem.checked !== undefined) {
        ...
        let where_append = event.currentTarget !== null && event.currentTarget !== undefined ? event.currentTarget.parentElement : null;
...
function validation_find_form(event) {
    ...
    let find_form = document.getElementById("find_form");
    let find_main_text_and_input_container = null;
    if (find_form) {
    ...
    if (!find_input) {
        ...
        for (let find_option of Array.from(find_options)) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (let checkbox_param of id_and_path_find) {
        ...
        if (checkbox_elem)
            checkbox_elem.addEventListener("click", function (event) { check_search_options(event, checkbox_param); });
    }
    if (find_button) {
        find_button.addEventListener("click", validation_find_form);
    }
    let find_input = document.getElementById("find_input");
});
