...
async function load_more_comments(event) {
    ...
    if (event) {
        ...
        if (this_element) {
            let last_ten_or_less_comments_id = new Array(10);
...
function add_event_listener_for_load_more_comments(load_more_comments_element) {
    if (!load_more_comments_element) {
        load_more_comments_element = document.querySelector(".load-more-comments");
    }
    if (load_more_comments_element) {
        load_more_comments_element.addEventListener("click", load_more_comments);
    }
}
function get_last_ten_or_less_comments_id(last_ten_or_less_comments_id, button_element) {
    let take_from = button_element && button_element.parentElement ? button_element.parentElement : document;
    let last_ten_or_less_comments_ids = Array.from(take_from.getElementsByClassName("comment-container"));
