declare function create_replies_button_of_comment(replies_count: number, comment_id: string): HTMLButtonElement;
declare function change_replies_count_by_replies_button_of_comment_text(count_replies_of_comment: HTMLButtonElement | null, replies_count: number):void;

async function create_reply_field_to_comment(this_element: HTMLButtonElement, comment_id: string)
{
...
function create_elements_for_write_reply(comment_container: HTMLDivElement, comment_id: string)
{
    ...
    cancel_write_reply.addEventListener("click", function(){let parent_element = cancel_write_reply.parentElement;
                                                            if(parent_element) parent_element.remove()});
...
async function submit_write_reply_comment(this_element: HTMLButtonElement, comment_id: string)
{
    this_element.disabled = true;
    ...
    if(comment_container)
    {
        ...
        {
            let reply_content_textarea_element = document.querySelector(".write-reply-content") as HTMLTextAreaElement;
            ...
            if(reply_content_textarea_element && reply_content_textarea_element.value && reply_content_textarea_element.value.trim().length > 0)
            {
                ...
                if(result.ok)
                {
                    ...
                    let replies_button_of_comment = comment_container.querySelector(".replies-button-of-comment") as HTMLButtonElement;
                    if(!replies_button_of_comment)
                    {
                        replies_button_of_comment = create_replies_button_of_comment(0, comment_id);
                        let dislike_button_of_comment = comment_container.querySelector(".dislike-button-of-comment") as HTMLButtonElement;
