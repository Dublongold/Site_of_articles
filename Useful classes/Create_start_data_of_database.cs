    ...
    public static class Create_start_data_of_database
    {
        public static void Create()
        {
            using (Database_context db_context = new())
            {
                ...
                db_context.Articles.Add(new Article
                {
                    Theme = "Theme 1",
                    Tags = "programming",
                    Description = "But who is the clinical bureau, that is made great",
                ...
                db_context.Articles.Add(
                    new Article
                    {
                        Theme = "Theme 2",
                        Description = "Chat as my therapist. I would like to spend the rest of my life, as well as to set up the vestibule, my various pools, as arrows, but now",
                ...
                /*
                    Articles_which_liked = db_context.Articles.Where(art => art.Id == 3).ToList(), // Dublon
                   
                    Articles_which_liked = db_context.Articles.Where(art => art.Id == 1 || art.Id == 3).ToList(),
                    Articles_which_disliked = db_context.Articles.Where(art => art.Id == 2).ToList() // Bob
                 
                 */
                db_context.User_accounts.AddRange(user_accounts);
                foreach (Article article in db_context.Articles.Where(art => art.Id < 4))
                {
                    if (article.Id == 1)
                    {
                        article.Users_who_react = new() { new() {Who_react_id = 2, Reaction_type = 1} };
                    }
                    else if (article.Id == 2)
                    {
                        article.Users_who_react = new() { new() { Who_react_id = 2, Reaction_type = 2 } };
                    }
                    else if (article.Id == 3)
                    {
                        article.Users_who_react = new() 
                        {
                            new() { Who_react_id = 2, Reaction_type = 1 },
                            new() { Who_react_id = 1, Reaction_type = 1 }
                        };
                    }
                }
                db_context.SaveChanges();
                db_context.Article_comments.Add(
                    new Article_comment()
                    {
                        ...
                        Users_who_react = new() { new() { Who_react_id = 1, Reaction_type = 2 } }
                    }); ;
                db_context.Article_comments.Add(
                    new Article_comment()
                    {
                ...
                db_context.Article_comments.Add(
                    new Article_comment()
                    {
                        ...
                        Author = db_context.User_accounts.First(ac => ac.Id == 2),
                        Content = "Good article!",
                        Users_who_react = new() { new() { Who_react_id = 1, Reaction_type = 1 } }
                    });
                db_context.Article_comments.Add(
                    new Article_comment()
                    {
